...
draft: False
type: how-to
description: 爲什麼這麼做呢？ 雲同步固然方便，可以讓你在任何設備中隨時開展工作，但這不是網盤發明的目的，也會帶來額外的性能開銷，增加設備發熱和網絡帶寬流量。 一個比較好的解決方案就是，保留你本地的副本，你依然可以在各個設備上開展工作，但是你的 git 目錄需要存在本地的某個目錄，Git 本身給了很好分離支持，如 --git-dir。 那麼接下來的問題就是如何把當前正常的 Git 項目中的 .git 目錄拆分出...
---
